      ...
      <div class="dynamic-content">
          <div class="avatar">
              <div class="avatar-container">
                <img @click="openHome(1,store.homeUserInformation.id)" :src="store.homeUserInformation.avatarAddress">
                <userInfo2 class="user-info" :user-info="store.homeUserInformation"/>
              </div>
          </div>
          <div class="head">
              <div class="user-name"><span @click="openHome(1,store.homeUserInformation.id)">{{ store.homeUserInformation.userName }}</span></div>
          ...
          <div v-if="dynamic.comment" class="body" @click="openDynamic(dynamic.dynamic.id)">
              ...
              <div v-if="dynamic.video" class="video-info-container" @click.stop>
                   <div class="up-info">
                       ...
                       <div class="right">
                            <div v-show="dynamic.upUserInfo2.isFollowFlag===0" class="follow" @click="followF(dynamic)">关注</div>
          ...
          <div v-if="dynamic.replyComment||dynamic.userInfo2s?.length>0" class="comment">
              <div class="comment-content">
                  ...
                  <div v-if="dynamic.replyComment" class="comment-text">
                      ...
                      <span class="user-content2-container">
          ...
          <div class="fotter">
              ...
              <div class="comment2" @click="openComment(dynamic)" :style="{color:store.commentOpenFlag===dynamic.dynamic.id?'#00AEEC':'#61666D'}">
          ...
          <div v-if="store.userInformation.id&&(store.userInformation.id===userId||store.userInformation.adminFlag)" class="up-btn">
            ...
            <div class="up-info">
               <div v-if="store.userInformation.id===userId" @click="putUpStatusF(dynamic.dynamic)">{{ dynamic.dynamic.upFlag===1?'取消置顶':'置顶' }}</div>
               <div @click="deleteDynamicDialogFlag=true">删除</div>
            </div>
          </div>
      </div>
      <comment2 v-if="dynamic.video&&!dynamic.comment&&store.commentOpenFlag===dynamic.dynamic.id" :dynamic="dynamic"/>
      <comment3 v-if="(dynamic.comment||!dynamic.dynamic.videoId)&&store.commentOpenFlag===dynamic.dynamic.id" :dynamic="dynamic"/>
      <el-dialog
...
import comment2 from '@/comment/comment2.vue';
import comment3 from '@/comment/comment3.vue';
...
// 关注
async function followF(dynamic){
...
if(store.userId===dynamic.upUserInfo2.id){
    ElMessage({
...
await addFollow(store.token,store.userId,dynamic.upUserInfo2.id).then(res=>{
    if(res.data.code===1){
        dynamic.upUserInfo2.isFollowFlag=1;
        dynamic.upUserInfo2.fansNumber++;
        ElMessage({
...
// 取消关注
async function unFollowF(dynamic){
...
await deleteFollow(store.token,store.userId,dynamic.upUserInfo2.id).then(res=>{
    if(res.data.code===1){
        dynamic.upUserInfo2.isFollowFlag=0;
        ElMessage({
        message: "取消了对 "+dynamic.upUserInfo2.userName+" 的关注",
        type: "info",
        plain: true,
        duration: 1700,
      });
    ...
    border: 1px solid #E3E5E7;
        ...
        position: relative;
            ...
            padding-right: 20px;
              ...
              .like:hover,.share:hover,.comment2:hover{
