<template>
  <div
    ...
    <div class="up-user-info-container-footer">
        ...
        ></span
        >
        <span class="up-user-info-container-footer-message" @click="addDialogueF(userInfo.id)">发消息</span>
    </div>
    </div>
  </div>
</template>
...
//关注用户
async function addFollowAxios(userInfo) {
        ...
        addFollow(store.token,store.userId,userInfo.id).then(res => {
          if(res.data.code === 1)
          {
          userInfo.isFollowFlag=1;
          userInfo.fansNumber++;
          }
      });
}
...
//取消关注用户
async function deleteFollowAxios(userInfo) {
        ...
        deleteFollow(store.token,store.userId,userInfo.id).then(response => {
          if (response.data.code === 1) 
          { 
            userInfo.isFollowFlag=0;
            userInfo.fansNumber--;
          }
      });
