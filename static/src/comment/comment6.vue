<template>
  <div class="comment-container"  v-loading.fullscreen="loading" element-loading-background="rgba(122, 122, 122, 0)">
    <div class="comment-header">
      ...
      >
        <span
          >请先<button @click="store.loginDialogVisibleFlag=store.loginDialogVisibleFlag===0?1:0">
            登录</button
          >后发表评论 (・ω・)</span
        >
      </div>
      ...
      >
        ...
        >
          ...
          <div v-show="commentImg.length > 0" class="comment-img-container"
            ref="replyImgContainer"
            @wheel.prevent="handleScrollImg"
            >
    ...
    <div
      ...
      >
        ...
        >
          ...
          >
          <span
            v-if="store.userId === comment.userInfo.id || store.userId===props.dynamic.dynamic.followId"
          ...
          >
            ...
            >
              ...
              <div
                ...
                @wheel.prevent="handleScrollImg2($event,comment.comments.id)"
              >
        ...
        <div v-else :style="{backgroundImage:`url(${remark.userInfo.backgroundAddress})`}" class="up-user-info-bg"></div>
          <div class="up-user-info-container">
            ...
            >
              ...
              >
              <span
                v-if="store.userId === remark.userInfo.id || store.userId===props.dynamic.dynamic.followId"
                class="comment-delete-btn-container"
          ...
          >
            ...
            >
              ...
              <div
                ...
                class="comment-img-container1"
                @wheel.prevent="handleScrollImg2($event,remark.comments.id)"
              >
                <div v-for="(file,index) in commentImg2" :key="file">
                  <span
                    v-show="file === null || file === undefined"
    ...
    <div v-show="scrollInputCommentFlag>(props.offSetHeight+160)&&store.userId!==null" class="comment-input-container2">
      <img
        @click="openHome(1,store.userId)"
        class="comment-avatar"
        v-show="store.userId !== null"
      ...
      >
        ...
        >
          ...
          <div v-show="commentImg.length > 0" class="comment-img-container" id="commentImg"
            ref="replyImgContainer2"
            @wheel.prevent="handleScrollImg3"
            >
            <div v-for="(file,index) in commentImg" :key="file" >
              <span
                v-show="file === null || file === undefined"
...
export default {
  ...
  setup(props) {
    ...
    if(scrollInputCommentFlag.value<(160+props.offSetHeight))
    ...
    async function publishComment() {
      ...
      let comments = {
        ...
        replyUserId: props.dynamic.dynamic.followId,
        upFlag: 0
      };
      ...
      if (res.data.code === 1) {
        ...
        props.dynamic.dynamic.commentNumber++;
    ...
    async function deleteCommentAxios() {
      ...
      if (res.data.code === 1) {
        ...
        {
          const index = commentList.findIndex((item) => item.comments.id === commentId.value);
          if(index!==-1)
            props.dynamic.dynamic.commentNumber=props.dynamic.dynamic.commentNumber-commentList[index].replyNumber-1;
    ...
    function handleScrollImg(e){
    if(!e.target||replyImgContainer.value===null)
    return;
    ...
    // 应用惯性衰减系数
    targetScrollLeft = replyImgContainer.value.scrollLeft + delta * speedFactor * 0.5;

    // 边界约束
    targetScrollLeft = Math.max(0, Math.min(
    targetScrollLeft,
    replyImgContainer.value.scrollWidth - replyImgContainer.value.clientWidth
    ));

    // 使用 requestAnimationFrame 平滑处理
    ...
    // 平滑滚动
    const smoothScroll = () => {
    if(!replyImgContainer.value)
    return;
    const current = replyImgContainer.value.scrollLeft;
    const diff = targetScrollLeft - current;

    // 达到目标位置或微小差异时停止
    if (Math.abs(diff) < 0.5) {
    replyImgContainer.value.scrollLeft = targetScrollLeft;
    return;
    }
    // 应用缓动函数 (easeOutQuad)
    const newPosition = current + diff * 0.15;
    replyImgContainer.value.scrollLeft = newPosition;
    animationFrame = requestAnimationFrame(smoothScroll);
    }

    let replyImgContaine=null;
    let targetScrollLeft2 = 0;
    let animationFrame2 = null;
    function handleScrollImg2(e,id){
     replyImgContaine=document.getElementById("replyImg"+id);
    if(!e.target)
    return;
    ...
    // 应用惯性衰减系数
    targetScrollLeft2 = replyImgContaine.scrollLeft + delta * speedFactor * 0.5;

    // 边界约束
    targetScrollLeft2 = Math.max(0, Math.min(
    targetScrollLeft2,
    replyImgContaine.scrollWidth - replyImgContaine.clientWidth
    ));
    ...
    if(!replyImgContaine)
    return;
    const current = replyImgContaine.scrollLeft;
    const diff = targetScrollLeft2 - current;

    // 达到目标位置或微小差异时停止
    if (Math.abs(diff) < 0.5) {
    replyImgContaine.scrollLeft = targetScrollLeft2;
    return;
    }
    // 应用缓动函数 (easeOutQuad)
    const newPosition = current + diff * 0.15;
    replyImgContaine.scrollLeft = newPosition;
    animationFrame2 = requestAnimationFrame(smoothScroll2);
    }

    //滑动图片
    let targetScrollLeft3 = 0;
    let animationFrame3 = null;
    function handleScrollImg3(e){
    if(!e.target||replyImgContainer2.value===null)
    return;
    ...
    // 应用惯性衰减系数
    targetScrollLeft3 = replyImgContainer2.value.scrollLeft + delta * speedFactor * 0.5;

    // 边界约束
    targetScrollLeft3 = Math.max(0, Math.min(
    targetScrollLeft3,
    replyImgContainer2.value.scrollWidth - replyImgContainer2.value.clientWidth
    ));

    // 使用 requestAnimationFrame 平滑处理
    if (animationFrame3) cancelAnimationFrame(animationFrame3)
    smoothScroll3();
    ...
    // 平滑滚动
    const smoothScroll3 = () => {
    if(!replyImgContainer2.value)
    return;
    const current = replyImgContainer2.value.scrollLeft;
    const diff = targetScrollLeft3 - current;

    // 达到目标位置或微小差异时停止
    if (Math.abs(diff) < 0.5) {
    replyImgContainer2.value.scrollLeft = targetScrollLeft3;
    return;
    }
    // 应用缓动函数 (easeOutQuad)
    const newPosition = current + diff * 0.15;
    replyImgContainer2.value.scrollLeft = newPosition;
    animationFrame3 = requestAnimationFrame(smoothScroll3);
    }
       ...
       if(res.data.code===1){
         if(res.data.data!==null){
           ...
           if(res.data.data.replyCommentList!==null&&commentList[0].comments.upFlag===0)
            ...
            setTimeout(() => {
             window.scrollTo({ top: props.offSetHeight, behavior: "smooth" });
            }, 500);
         }
       }
       })
