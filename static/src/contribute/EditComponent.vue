  ...
  <div class="main-content4" v-loading="loading">
    <div class="white">
      ...
      <div
        ...
        "
      >
        <div style="font-size: 14px; transform: translate(50px, 60px)">
          {{ fileVideoname }}
        </div>
        <div
          style="
        ...
        <div style="transform: translate(210px, -15px)">
          ...
          >
            <template #reference>
              <el-button
                class="m-2"
                style="border: none; font-size: 12px; color: #9c9f9f"
                >视频大小</el-button
              >
            </template>
          </el-popover>
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      getEitList();
      videoEditId.value = route.query.videoId;

      if (videoEditId.value !== undefined) {
        selectByVideoId();
      }
    ...
    async function selectByVideoId() {
      try {
        const response = await apiClient.get("/video/getVideoInfo", {
          params: {
            videoId: videoEditId.value,
          },
          headers:{
    ...
    const uploadFile = async () => {
      ...
      try {
        const response = await apiClient.put(
          "http://localhost:8081/upload/updateVideo",
          formData,
          {
            headers: {
