<template>
  <div v-if="store.userInformation.adminFlag||store.userInformation.userName==='MoJi'" class="main-content">
    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer" style="transform: translate(-10px, 50px)">
          <el-button class="yesDelete" @click="examineVideo">确定</el-button>
          <el-button
            type="primary"
            @click="videoExamineFailed()"
            style="
              background-color: white;
              color: black;
              border: 1px solid #b3b3b3;
          ...
          >
          <el-input
            @keyup.enter="videoExamineFailedAxios()"
            placeholder="请输入退回理由"
            v-if="examineFiledMessageDisplay"
            v-model="examineFiledMessage"
            maxlength="20"
            type="textarea"
...
export default {
  ...
  setup() {
    ...
    const examineFiledMessageDisplay = ref(false);
    const examineFiledMessage = ref("");
    ...
    watch(dialogVisible1, (newValue) => {
      if (newValue === false) examineFiledMessageDisplay.value = false;
    });

    async function selectAllUploadVideo() {
      try {

        const response = await apiClient.get("/video/userExamineVideo", {
          params: {
            userId: store.userId,
    ...
    async function videoExamineFailedAxios() {
      try {
        const response = await apiClient.post("/video/examineVideoFiled", {
          videoId: videoTemp.id,
          userId: store.userId,
          examineFiledMessage: examineFiledMessage.value,
        },
        {
            headers: {
            "Content-Type": "application/json",
    ...
    function videoExamineFailed() {
      examineFiledMessageDisplay.value = true;
    }

    function examine(video) {
      Object.assign(videoTemp, video);
    ...
    return {
      ...
      examineVideo,
      examineFiledMessage,
      examineFiledMessageDisplay,
      videoExamineFailed,
      videoExamineFailedAxios,
      store,
      openVideo,
    };
  },
